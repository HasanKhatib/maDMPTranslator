...
namespace maDMPTranslator.Logic
{
    public class DMPHorizonLogic : IDMPLogic
    {
        ...
        private Models.RDA_DMP.maDMP maDMP { set; get; }
        ...
        public Dictionary<string, List<string>> InitializeAnswersDict(Models.RDA_DMP.maDMP maDMP)
        {
            ...
            //13 dmp_description
            AnswersDict["DATA_SUMMARY"] = new List<string>() {string.Format("In this experiment, {1}." +
                                                                "\n With this purpose, the following datasets are required:" +
                                                                "\n {2}." +
                                                                "\n The information contained is in {3} language formatted as {4} about {5}." +
                                                                " It does {6} contain sensitive data as well as {7} regarding personal data. " +
                                                                "{8}.\n This dataset has been issued on: {9}." +
                                                                "\n The data file has its own Digital Identification {10} which can be accessed through the link " +
                                                                "{11}, its total size is of {12} bytes.\n",
                                                                maDMP.DMP.Description,
                                                                string.Join(",", maDMP.DMP.Dataset),
                                                                maDMP.DMP.Language,
                                                                maDMP.DMP.Dataset.First()?.DatasetType,
                                                                maDMP.DMP.Dataset.First()?.Description,
                                                                maDMP.DMP.Dataset.First()?.SensitiveData,
                                                                maDMP.DMP.Dataset.First()?.PersonalData,
                                                                maDMP.DMP.Dataset.First()?.PreservationStatement,
                                                                maDMP.DMP.Dataset.First()?.Issued,
                                                                maDMP.DMP.Dataset.First()?.DatasetTypeIdentifier.IdentifierType,
                                                                maDMP.DMP.Dataset.First()?.DatasetTypeIdentifier.ID,
                                                                maDMP.DMP.Dataset.First()?.DistributionList.First().ByteSize
                                                                ),
